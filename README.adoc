...
:icons: font
:apidoc_url: https://tibhannover.gitlab.io/orkg/orkg-backend/index.html
:apiclientdoc_url: https://tibhannover.gitlab.io/orkg/orkg-backend/client-libraries/client-libraries.html
...
This project requires Java and Podman/Docker to be installed on the development system.
Other software, e. g. the Gradle build system, will be downloaded when required.

On Linux, https://mise.jdx.dev/[Mise-en-place] can be used to install software required to develop and run this project.
After installation, run `mise install` to fetch Java other software.

To make use of the installed versions, run commands via `mise exec`, e. g. `mise exec -- ./gradlew build`.

The default configuration prefers Podman over Docker.
If you do not like that change or run into problems, you can override the settings in `mise.local.toml`.
There is a known incompatibility when using Podman with Testcontainers, resulting in sporadic test failures when running Integration Tests.
As a workaround you need to increase the default `service_timeout` of your Podman instance.
This can be done by running the following command for your operating system:
...
This project provides two auto-generated client libraries, based on OpenAPI.
The version of the client library matches the version of the backend API.
The documentation for the client liberaries can be found at {apiclientdoc_url}.
