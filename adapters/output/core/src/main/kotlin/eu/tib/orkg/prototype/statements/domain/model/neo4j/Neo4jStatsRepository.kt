...
interface Neo4jStatsRepository : Neo4jRepository<Neo4jResource, Long> {
    ...
    @Query("""MATCH(sub: Resource) WHERE ('Paper' IN LABELS(sub) OR 'Comparison' IN LABELS(sub) OR 'Problem' IN LABELS(sub) OR 'Contribution' IN LABELS(sub) OR 'Visualization' IN LABELS(sub)) AND (sub.created_by <> '00000000-0000-0000-0000-000000000000' AND sub.created_at > {0} ) RETURN DISTINCT sub.created_by AS id, count(sub) AS contributions SKIP $skip LIMIT $limit""",
