...
import org.orkg.contenttypes.adapter.input.rest.TableRepresentation.RowRepresentation
...
interface TableRepresentationAdapter : ThingReferenceRepresentationAdapter {
    ...
    fun Row.toRowRepresentation(): RowRepresentation =
        RowRepresentation(
            label = label,
            data = data.map { thing -> thing?.let { ThingReference.from(it).toThingReferenceRepresentation(false) } },
        )
}
