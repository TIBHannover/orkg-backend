...
RETURN COUNT(ds) AS cnt"""
    ...
    @Query(
        """
MATCH (ds:$DATASET_CLASS {id: $ID})<-[:RELATED {predicate_id: '$DATASET_PREDICATE'}]-(b:$BENCHMARK_CLASS)<-[:RELATED {predicate_id: '$BENCHMARK_PREDICATE'}]-(c:Contribution)-[:RELATED {predicate_id: 'P32'}]->(:Problem {id: $PROBLEM_ID})
