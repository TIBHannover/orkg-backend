...
internal class PaperAuthorUpdaterUnitTest : MockkBaseTest {
    ...
    fun `Given a paper update command, it updates the authors`() {
        ...
        every { authorUpdater.update(state.statements, command.contributorId, state.authors, command.paperId, command.extractionMethod!!) } just runs

        paperAuthorUpdater(command, state)

        verify(exactly = 1) { authorUpdater.update(state.statements, command.contributorId, state.authors, command.paperId, command.extractionMethod!!) }
    }
