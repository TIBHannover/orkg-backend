...
internal class TemplateInstanceTemplateUpdateValidatorUnitTest : MockkBaseTest {
    private val templateService: TemplateUseCases = mockk()

    private val templateInstanceTemplateUpdateValidator = TemplateInstanceTemplateUpdateValidator(templateService)
    ...
    fun `Given a template instance update command, when validating its template, it returns success`() {
        ...
        val result = templateInstanceTemplateUpdateValidator(command, state)
    ...
    fun `Given a template instance update command, when template is not found, it throws an exception`() {
        ...
        shouldThrow<TemplateNotFound> {
            templateInstanceTemplateUpdateValidator(command, state)
        }
