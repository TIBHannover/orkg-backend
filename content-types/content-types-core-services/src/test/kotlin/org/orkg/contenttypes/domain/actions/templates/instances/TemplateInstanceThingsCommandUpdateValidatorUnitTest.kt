...
internal class TemplateInstanceThingsCommandUpdateValidatorUnitTest : MockkBaseTest {
    private val thingsCommandValidator: ThingsCommandValidator = mockk()

    private val templateInstanceThingsCommandUpdateValidator = TemplateInstanceThingsCommandUpdateValidator(thingsCommandValidator)
    ...
    fun `Given a template instance update command, when validating its thing commands, it returns success`() {
        ...
        val result = templateInstanceThingsCommandUpdateValidator(command, state)
