...
This reference aims to explain the patterns used.
...
The most important package is the `$\{context}-core-model` module.
It contains all domain objects.

Alongside the `$\{context}-core-model` package lives the `$\{context}-core-services` module.
...
The modules `$\{context}-ports-input` and `$\{context}-ports-output` contain the input and output ports, respectively.
Input ports are interfaces ending in the name `UseCase` that provide methods for a specific use case.
Output ports are interfaces ending in `Repository`.

All classes and interfaces in the `$\{context}-core-model` module form the API and therefore need to be public.

==== Adapter Modules

The input and output adapters are not part of the application logic, so they are contained in their own module.
They are further split into separate concerns.
For example, all Spring Controllers live in the `$\{context}-adpater-input-rest-spring-mvc` module as they are adapters to the input ports of the application.
The JPA and OGM entities as well as Spring repositories live in the `$\{context}-adpater-output-spring-data-jpa` and `$\{context}-adpater-output-spring-data-neo4j` respectively.
...
flowchart TD
    ...
    subgraph Input
        subgraph adapter-input-rest-spring-mvc
            FooController
        end
    ...
    FooRepositoryAdapter -.->|implements| FooRepository

    FooService --> |accesses| Foo
    FooService -.-> |implements| FooUseCase
    FooService --> |accesses| FooRepository

    FooController --> |accesses| FooUseCase
----

It is located in the `core-model` module.
...
The service accesses the `FooRepository` to retrieve `Foo`s from an "external" system.
...
The `FooRepositoryAdapter` provides an implementation of the `FooRepository` to find and load `Foo`s in a database (or other system).
