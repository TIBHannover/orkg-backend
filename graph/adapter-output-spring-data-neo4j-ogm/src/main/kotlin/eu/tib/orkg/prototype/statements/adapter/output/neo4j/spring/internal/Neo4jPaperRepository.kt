...
interface Neo4jPaperRepository : Neo4jRepository<Neo4jResource, Long> {
    ...
    @Query(value = """MATCH p=(paper:Paper:Resource)-[:RELATED*]->(:Resource{resource_id: $id}) WITH paper, apoc.coll.flatten([r in relationships(p) | [startNode(r), r]]) AS path UNWIND path AS thing MATCH (t:Thing) WHERE t.resource_id = thing.resource_id OR t.predicate_id = thing.predicate_id RETURN paper, COLLECT(t) AS path""",
