...
class DiscussionService(
    ...
    override fun findByTopicAndCommentId(topic: ThingId, id: DiscussionCommentId): Optional<DiscussionCommentRepresentation> =
        thingRepository.findByThingId(topic)
            .filter { it !is Literal }
            .map { repository.findById(id).map { it.toDiscussionCommentRepresentation() } }
            .orElseThrow { TopicNotFound(topic) }

    override fun findAllByTopic(topic: ThingId, pageable: Pageable): Page<DiscussionCommentRepresentation> =
        thingRepository.findByThingId(topic)
            .filter { it !is Literal }
            .map { repository.findAllByTopic(topic, pageable).map { it.toDiscussionCommentRepresentation() } }
            .orElseThrow { TopicNotFound(topic) }
    ...
    private fun String?.isValid(): Boolean {
        ...
        val stripped = this.replace(orkgUrlPattern, "")
        val matcher = urlPattern.matcher(stripped)
        return !matcher.find()
    }
}

fun DiscussionComment.toDiscussionCommentRepresentation(): DiscussionCommentRepresentation = object : DiscussionCommentRepresentation {
