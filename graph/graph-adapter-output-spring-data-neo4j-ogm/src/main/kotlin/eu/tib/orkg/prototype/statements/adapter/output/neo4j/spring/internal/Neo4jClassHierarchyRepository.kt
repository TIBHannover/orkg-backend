...
interface Neo4jClassHierarchyRepository : Neo4jRepository<Neo4jClass, Long> {
    ...
    @Query("""MATCH (c:Class {id: $id})-[:$relation*0..]->(p:Class) WITH collect(p) + c AS classes UNWIND classes AS class WITH DISTINCT class OPTIONAL MATCH (class)-[:$relation]->(p:Class) RETURN class, p.id AS parentId ORDER BY class.id ASC""",
        countQuery = """MATCH (c:Class {id: $id})-[:$relation*0..]->(p:Class) WITH collect(p) + c AS classes RETURN COUNT(DISTINCT classes)""")
