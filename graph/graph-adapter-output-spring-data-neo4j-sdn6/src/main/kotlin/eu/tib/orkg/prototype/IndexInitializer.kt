...
class IndexInitializer : InitializingBean {
    ...
    override fun afterPropertiesSet() {
        with (neo4jClient) {
            ...
            query("""CREATE FULLTEXT INDEX fulltext_idx_for_predicate_on_label IF NOT EXISTS FOR (n:Predicate) ON EACH [n.label]""").run()
            query("""CREATE FULLTEXT INDEX fulltext_idx_for_literal_on_label IF NOT EXISTS FOR (n:Literal) ON EACH [n.label]""").run()
        }
    }
}
