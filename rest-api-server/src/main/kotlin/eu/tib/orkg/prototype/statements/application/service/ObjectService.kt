...
class ObjectService(
    ...
    fun createObject(
        ...
        // Get provenance info
        val userId = ContributorId(contributorUUID)
            ...
            for (jsonObject in value) {
                when {
                    ...
                    jsonObject.isNewResource() -> { // create new resource
                        ...
                        MAP_PREDICATE_CLASSES[predicateId!!.value]?.let { ClassId(it) }
                            ?.let { classes.add(it) }
                        // Create resource
...
data class CreateObjectRequest(
     ...
     */
    fun hasTempPredicates() = this.predicates != null && this.predicates.isNotEmpty()
}
...
data class NamedObject(
     ...
     */
    fun hasSubsequentStatements() = this.values != null && this.values.isNotEmpty()

    /**
     * Check if the resource is typed
     * i.e., it has classes
     */
    fun isTyped() = this.classes != null && this.classes.isNotEmpty()
}
...
data class ObjectStatement(
     ...
     */
    fun isExistingResource() = this.isExisting() && when (this.`@type`) {
     ...
     */
    fun isExistingLiteral() = this.isExisting() && when (this.`@type`) {
     ...
     */
    fun isExistingClass() = this.isTypedExisting() && this.`@type` == CLASS_EXISTING_KEY

    /**
     * Check if the entity is existing as a predicate
     * If it is typed with predicate
     */
    fun isExistingPredicate() = this.isTypedExisting() && this.`@type` == PREDICATE_EXISTING_KEY

    /**
     * Check if the resource is a temp resource
     * i.e., its id starts with an _
     */
    fun isTempResource() = this.`@id`!!.startsWith("_")
