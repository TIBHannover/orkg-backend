...
class ContributorController(
    ...
    @GetMapping("/{id}")
    fun findContributorByUserId(@PathVariable id: ContributorId): ResponseEntity<Contributor> {
        val contributor = contributorService.findById(id).orElseThrow { UserNotFound("$id") }
        return ResponseEntity.ok(contributor)
    }

    @GetMapping("/{id}/organizations")
    fun findMemberOrganizationsByUserId(@PathVariable id: ContributorId): Iterable<Contributor> =
