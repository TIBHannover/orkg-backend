...
class DOIController(
    ...
    fun addDOI(@RequestBody doiData: CreateDOIRequest): String? {
        ...
        //var a = "$dataciteTestUsername:$dataciteTestPassword"
        //createXmlMetadata(doiData.comparisonId, doiData.description, doiData.title, doiData.subject)
        ...
        //doiData.authors.forEach { println(it.creator +" "+ it.ORCID) }
    ...
    fun getCreatorsXml(authors: Set<Creator>): String {
        ...
        authors.forEach {
            ...
            "</creator\n"
            //println(it.creator+" "+it.ORCID)
        }
        return authorsList
        //authors.forEach { println(it.creator+" "+it.ORCID) }
    }

    fun getRelatedPapers(relatedResources: Set<ResourceId>): String {
        val pagination = PageRequest.of(0, 1)
        var relatedIdentifiers=""
        //var s = statementService.findAllBySubject(, pagination)
        //var result = null
        relatedResources.forEach {
            ...
            for (statement in statementResult) {
                var paper = refreshObject(statement.subject)
                var result = statementService.findAllBySubjectAndPredicate(paper.id.toString(), PredicateId(ID_DOI_PREDICATE), pagination)
                for(res in result) {
                    //println(refreshObject1(res.`object`).label)
                    relatedIdentifiers+="<relatedIdentifier relationType=\"IsDerivedFrom\" relatedIdentifierType=\"DOI\">${refreshObject1(res.`object`).label}</relatedIdentifier>\n"
                }
                //(result.forEach { it-> println(it.id) })
            }
            //result.forEach { it -> println(it.subject) }
            //println(it.value)
        }
        return relatedIdentifiers
    }

    private fun refreshObject(thing: Thing): Resource {
        return when (thing) {
            is Resource -> thing
            else -> error("")
        }
    ...
    private fun refreshObject1(thing: Thing): Literal {
        return when (thing) {
            is Literal -> thing
            else -> error("")
        }
