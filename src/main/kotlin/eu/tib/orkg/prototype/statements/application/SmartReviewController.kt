...
@RestController
@RequestMapping("/api/smartreviews")
class SmartReviewController(
    ...
    fun getNonFeaturedSmartReviews(pageable: Pageable) =
        neo4jResourceService.loadNonFeaturedSmartReviews(pageable)
    ...
    fun getUnlistedSmartReviews(pageable: Pageable) =
        neo4jResourceService.loadUnlistedSmartReviews(pageable)

    @GetMapping("/metadata/unlisted", params = ["unlisted=false"])
    fun getListedSmartReviews(pageable: Pageable) =
        neo4jResourceService.loadListedSmartReviews(pageable)
