...
interface Neo4jStatsRepository : Neo4jRepository<Neo4jResource, Long> {
    ...
    @Query("""MATCH (comp:Comparison)-[:RELATED*0..1]->(contribution:Contribution)<-[:RELATED*0..1]-(p:Paper)-[:RELATED*0..1]->(r:ResearchField{resource_id: {0}}) OPTIONAL MATCH(v:Visualization)<-[:RELATED*0..1]-(comp) WITH COLLECT([v.resource_id,v.label, v.created_at, v.created_by,labels(v)])  +  COLLECT([comp.resource_id,comp.label, comp.created_at, comp.created_by, labels(comp)]) + COLLECT([contribution.resource_id,contribution.label, contribution.created_at, contribution.created_by, labels(contribution)]) + COLLECT([p.resource_id,p.label, p.created_at, p.created_by, labels(p)]) + COLLECT([r.resource_id,r.label, r.created_at, r.created_by, labels(r)]) AS items UNWIND items AS changelogs RETURN DISTINCT changelogs[0] AS id, changelogs[1] AS label, changelogs[2] AS createdAt, changelogs[3] AS createdBy, changelogs[4] AS classes""",
    countQuery = "MATCH (comp:Comparison)-[:RELATED*0..1]->(contribution:Contribution)<-[:RELATED*0..1]-(p:Paper)-[:RELATED*0..1]->(r:ResearchField{resource_id: {0}}) OPTIONAL MATCH(v:Visualization)<-[:RELATED*0..1]-(comp) WITH COLLECT([v.resource_id,v.label, v.created_at, v.created_by,labels(v)]) + COLLECT([comp.resource_id,comp.label, comp.created_at, comp.created_by, labels(comp)]) + COLLECT([contribution.resource_id,contribution.label, contribution.created_at, contribution.created_by, labels(contribution)]) + COLLECT([p.resource_id,p.label, p.created_at, p.created_by, labels(p)]) + COLLECT([r.resource_id,r.label, r.created_at, r.created_by, labels(r)]) AS items UNWIND items AS changelogs RETURN COUNT(DISTINCT changelogs) AS cnt")
